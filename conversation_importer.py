    ...
    def parse_conversation_script(self, file_path: str) -> List[Dict]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if not parsed and line:
                print(f"⚠️  Line {line_num} couldn't be parsed: {line[:50]}...")
        
        print(f"📊 Parsed {parsed_count} messages from {len(lines)} lines")
        return messages
    
    def import_conversation(self, file_path: str, delay_between_messages: float = 0.1) -> bool:
        ...
        print(f"📋 Loaded {len(self.users)} existing users: {list(self.users.keys())}")
        ...
        for i, message in enumerate(messages, 1):
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    print(f"❌ Failed to import message {i} from {speaker}: {response.status_code}")
        ...
        print(f"\n🎯 IMPORT COMPLETE")
        print(f"✅ Successfully imported: {successful_imports}/{len(messages)} messages")
        print(f"👥 Users involved: {len(self.users)}")
        print(f"📊 Import success rate: {(successful_imports/len(messages))*100:.1f}%")
        ...
        log_data = {
            ...
            'users_created': [user for user in unique_speakers if user in self.users],
...
def main():
    ...
    print("2. Update the 'conversation_file' variable above with your file path")
    ...
    if not os.path.exists(conversation_file):
        ...
        print("📝 Please create a conversation script file with the format above")
    ...
    if success:
        ...
        print("💡 You can now run the question evaluator to test the AI's knowledge")
    else:
